...
const search = ref("");
const { fetchData, confirmDelete, deleteMutation } = useElectronicTypes();

const { isLoading, isError, refetch, data, error } = useQuery<
  TypeElectronicType[]
>({
  queryKey: ["electronic_types"],
  queryFn: fetchData,
});
...
const filteredData = computed(() => {
  ...
  return data.value.filter((item: TypeElectronicType) =>
...
<template>
  <div class="flex flex-col gap-4 w-full">
    ...
    <el-card shadow="never">
      <el-page-header>
        <template #content>
          <span class="text-large font-600 mr-3">ប្រភេទគ្រឿង</span>
        </template>
        <template #extra>
          <Create />
        </template>
    ...
    <el-card shadow="never">
      ...
      <el-table v-else :data="filteredData" style="width: 100%">
        ...
        <el-table-column prop="name" label="ឈ្មោះប្រភេទគ្រឿង" sortable />
